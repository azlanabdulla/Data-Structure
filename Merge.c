...
void
Swap (struct HeapNode *a, struct HeapNode *b)
{
...
void
HeapifyDown (struct HeapNode heap[], int n, int i)
{
  int sm = i;
  int l = 2 * i + 1;
  int r = 2 * i + 2;
...
void
MergeSortLists (int lists[10][10], int size[], int k)
{
...
{
  int k;
  printf ("Enter no.of sorted lists: ");
  scanf ("%d", &k);
  int lists[10][10];
  int size[10];
  for (int i = 0; i < k; i++)
    {
      printf ("Enter the no.of Elements in List %d:  ", i + 1);
      scanf ("%d", &size[i]);
